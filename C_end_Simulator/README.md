...
├── docker-compose.yml                  <-- Docker 编排配置（engine + tui + flask_server 服务定义与卷映射）
...
│   ├── app.py                          <-- ✅ Flask 主应用（接收 Engine POST 数据、健康检查接口）
│   ├── requirement.txt                 <-- Flask 依赖清单（flask）
│   │
│   └── storage/                        <-- ✅ [数据存储层：策略模式]
│       ├── __init__.py
│       ├── base_storage.py             <-- ✅ 存储抽象基类（BaseStorage ABC）
│       ├── file_storage.py             <-- ✅ 文件存储（JSONL 格式保存接收到的数据）
│       └── mysql_storage.py            <-- 🔮 未来占位：MySQL 数据库存储
│
